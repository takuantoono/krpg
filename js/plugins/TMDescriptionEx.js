 ...
 *   プラグインコマンドはありません。
 * 
 *   このプラグインは RPGツクールMV Version 1.3.1 で動作確認をしています。
 *
 *
...
(function() {
  ...
  Window_DescriptionEx.prototype.drawSkillParameters = function(x, y) {
    var lineHeight = this.lineHeight();
    this.changeTextColor(this.systemColor());
    this.drawText("会心", x, y + lineHeight * 0, TMPlugin.DescriptionEx.LeftPaneWidth);
    this.drawText("属性", x, y + lineHeight * 1, TMPlugin.DescriptionEx.LeftPaneWidth);
    ...
    if(this._item.damage&&this._item.damage.critical){
    this.drawText("○", x, y + lineHeight * 0, TMPlugin.DescriptionEx.LeftPaneWidth, 'right');
    }
    this.drawText(this._item.meta.eleNames, x, y + lineHeight * 1, TMPlugin.DescriptionEx.LeftPaneWidth, 'right');
    text = TMPlugin.DescriptionEx.OccasionValue[this._item.occasion];
    if(this._item.meta.inEvent){
    this.drawText("イベント", x, y + lineHeight * 2, TMPlugin.DescriptionEx.LeftPaneWidth, 'right');
    }else{
    if(this._item.meta.liangge){
    this.drawText("戦・平", x, y + lineHeight * 2, TMPlugin.DescriptionEx.LeftPaneWidth, 'right');
    }else{
    ...
    this.drawBattleItemParameters(x, y + lineHeight * 4);
  };
  ...
  Scene_Shop.prototype.createBuyWindow = function() {
    ...
    $gameVariables.setValue(155, true);
