...
Window_EquipSlot.prototype.drawItem = function(index) {
    ...
    this.drawText(this.slotName(index), rect.x, rect.y, ww1 -50);
    var ww2 = rect.width - ww1;
    var item = this._actor.equips()[index];
    if (item) {
      this.drawItemName(item, rect.x + ww1-50, rect.y, ww2);
    } else {
...
Window_EquipSlot.prototype.setInfoWindow2 = function(infoWindow) {
    this._infoWindow2 = infoWindow;
    this.update();
};
...
Window_EquipSlot.prototype.updateHelp = function() {
    ...
    if (SceneManager._scene instanceof Scene_Equip && this._infoWindow2) {
      this._infoWindow2.setItem(this.item());
    }
};
...
Window_EquipItem.prototype.setInfoWindow2 = function(infoWindow) {
    this._infoWindow2 = infoWindow;
    this.update();
};
...
Window_StatCompare.prototype.refresh = function() {
    ...
    if (!this._actor) return;
    for (var i = 0; i < 8; ++i) {
        this.drawItem(0, (this.lineHeight()-3) * i, i);
    }
};
...
y -= 5
    this.drawDarkRect(x, y, this.contents.width, this.lineHeight()-5);
...
Scene_Equip.prototype.createInfoWindow = function() {
    ...
    this._infoWindow2 = new Window_ShopInfo(wx, wy, ww, wh);
    this._slotWindow.setInfoWindow2(this._infoWindow2);
    this._itemWindow.setInfoWindow2(this._infoWindow2);
