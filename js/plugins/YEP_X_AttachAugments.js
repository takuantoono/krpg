...
ItemManager.augmentRefreshParty = function(item) {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      //alert(item.name)
      //if(item.name.contains("�n")) item.params[0]+=12;
      member.refresh();
      }
    }
};
...
ItemManager.applyAugmentSetSuffix = function(mainItem, text, slot, add) {
	...
	var oid = mainItem.id;
	if(mainItem.baseItemId)oid = mainItem.baseItemId;
    if(mainItem.originalBaseItemId)oid = mainItem.originalBaseItemId;
    if(oid<1)return;
    if(DataManager.isWeapon(mainItem)){
...
Window_AugmentItemList.prototype.initialize = function(x, y, width, height) {
    Window_ItemList.prototype.initialize.call(this, x, y, width, height -72);
    this._item = null;
    this._slotId = -1;
    this.hide();
    this.deactivate();
...
Scene_Item.prototype.createAugmentListWindow = function() {
    ...
    var wh = this._itemWindow.height + 8;
