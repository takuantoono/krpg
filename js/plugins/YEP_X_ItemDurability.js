...
DataManager.countOr = function(item) {
    if (this.isItem(item)) return -1;
    if (!this.isIndependent(item)) return -1;
    if (item.durability === undefined) ItemManager.makeDurability(item);
    return item.meta.no_count;
};
...
DataManager.getMaxDurability = function(item) {
    ...
    var olden = 0;
    if(item._olden)olden=item._olden;
    return baseItem.durMax-olden;
};

DataManager.getMaxOriDurability = function(item) {
    if (this.isItem(item)) return -1;
    if (!this.isIndependent(item)) return -1;
    var baseItem = this.getBaseItem(item);
...
ItemManager.clampBuseki = function(item) {
    var baseItem = DataManager.getBaseItem(item);
    item.buseki = item.buseki.clamp(0, 9999);
};
...
Game_Actor.prototype.damageRandomDurability = function(value, group) {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      if (obj.durability < 0) continue;
      valid.push(obj)
      if(obj.atypeId==5||obj.atypeId==6){
      if(Math.randomInt(3)>1)i=10
      }
    }
...
Game_Actor.prototype.damageShieldDurability = function(item) {
	if($gameActors.actor(1).isStateAffected(187)) return;
    if (!item) return;
    item.durability -= 1;
    if (item.durability <= 0) this.durabilityBreakItem2(item);
};

Game_Actor.prototype.damageWeaponDurability = function(item, num) {
	if($gameActors.actor(1).isStateAffected(187)) return;
    if (!item) return;
    item.durability -= 1;
    item.durability -= Math.randomInt(2);
    if (item.durability <= 0) this.durabilityBreakItem3(item, num);
};

Game_Actor.prototype.durabilityBreakItem3 = function(obj, num) {
    if (!obj) return;
    //this.discardEquip(obj);
    if(obj._broken) return;
    obj._broken = true;
    obj.durability = 0;
    $gameActors.actor(this._actorId).changeEquipById(num, 0)
    this.playDurabilityBreakSound(obj);
    this.customDurabilityBreakEval(obj);
    var scene = SceneManager._scene;
    var win = scene._logWindow;
...
Game_Actor.prototype.durabilityBreakItem2 = function(obj) {
    if (!obj) return;
    this.discardEquip(obj);
    this.playDurabilityBreakSound(obj);
    this.customDurabilityBreakEval(obj);
...
Game_Action.prototype.applyDurabilityEffects = function(target) {
    if (this.subject().isActor()) {
      ...
      if(value <= -1 && !this._dualWieldTargets && this.subject()._equips[0]){
      	var weapon = $dataWeapons[this.subject()._equips[0]._itemId];
      	this.subject().damageWeaponDurability(weapon, 1);
      }
      if(this.subject()._equips[1]){
      	if(this.subject()._equips[1]._dataClass=="armor"){
      		var shield = $dataArmors[this.subject()._equips[1]._itemId];
      		if(shield&&this._item.isSkill()&&shield.meta.no_count==this._item._itemId){
      			this.subject().damageShieldDurability(shield);
      		}
      	}else{
      		var weapon = $dataWeapons[this.subject()._equips[1]._itemId];
      		if(value<=-1&&weapon && this._dualWieldTargets){
      			this.subject().damageWeaponDurability(weapon, 2);
      		}
      	}
      };
...
Window_ItemInfo.prototype.drawItemDurability = function(dy) {
    ...
    if (cur > 0) {
      if(DataManager.countOr(this._item)){
      this.changeTextColor(this.textColor(this.durabilityColor3(cur, max)))
      }else{
...
Window_ItemInfo.prototype.durabilityColor3 = function(cur, max) {
    return Yanfly.Param.IDurColor['rate100'];
};
