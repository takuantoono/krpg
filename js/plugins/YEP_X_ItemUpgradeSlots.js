...
ItemManager.applyIUSEffects = function(mainItem, effectItem) {
    ...
    var line = effectItem.upgradeEffect[0];
...
ItemManager.processIUSEffect = function(line, mainItem, effectItem) {
    ...
    if (line.match(/BASE NAME:[ ](.*)/i)) {
      ...
      var value = $gameActors.actor(9)._name;
...
ItemManager.effectIUSParamRateChange = function(item, stat, value) {
    ...
    switch (stat) {
      ...
      case 'ATK':
      case 'STR':
        item.params[2] += Math.round(value * 0.01 * baseItem.params[2]);
        break;
      case 'DEF':
        item.params[3] += Math.round(value * 0.01 * baseItem.params[3]);
        break;
      case 'MAT':
      case 'INT':
      case 'SPI':
        item.params[4] += Math.round(value * 0.01 * baseItem.params[4]);
        break;
      case 'MDF':
      case 'RES':
        item.params[5] += Math.round(value * 0.01 * baseItem.params[5]);
        break;
      case 'AGI':
      case 'SPD':
        item.params[6] += Math.round(value * 0.01 * baseItem.params[6]);
        break;
      case 'LUK':
        item.params[7] += Math.round(value * 0.01 * baseItem.params[7]);
        break;
      case 'ALL':
        for (var i = 0; i < 8; ++i) {
...
Window_ItemInfo.prototype.drawSlotsInfo = function(dy) {
    ...
    if (item.originalUpgradeSlots) {
      ...
      text = '/' + 9;//Yanfly.Util.toGroup(baseItem.upgradeSlots);
    }
...
Scene_Item.prototype.createUpgradeListWindow = function() {
    ...
    var wh = this._itemWindow.height - this._itemWindow.fittingHeight(1) + 20;
